...
void loadNonEquippables() {
    ...
    for (const auto& item : json) {
        ...
        all_items[id] = std::make_shared<Object>(id, name, type);
    }
    file.close();
}

void loadWeapons() {
    std::ifstream file("weapons.json");
    nlohmann::json json;
    file >> json;
    ...
    for (const auto& item : json) {
        ...
        std::string magic_type = "none";
        if (magical) {
            magic_type = item["magic type"];
        }
        int enchantability = item["enchantability"];
        // int id, const std::string& name, const std::string& type, const std::string& description, float base_damage,
        //           float base_durability, float base_range, bool magical, const std::string& magic_type, int enchantability);
        all_items[id] = std::make_shared<Weapon>(id, name, "weapon", description, base_atk_dmg, base_durability, base_range, magical, magic_type, enchantability);
    }
    file.close();
}
